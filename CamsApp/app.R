...
ui = fluidPage(
    fluidRow(
        column(1),
        column(3,
               pickerInput("cities", NULL, choices = cities, options = list(`actions-box` = TRUE), multiple = T)),
        column(3,
               selectInput("ind", NULL, c(monthlyRaceName,
                                          yearlyRaceName,
                                          "Time series", 
                                          "Rank",
                                          "Rank change from Jan 2016 = 0",
                                          "Index (Jan 2016 = 1.0)"))),
        column(2,
    ...
    fluidRow(
      column(1),
      column(8, div(tags$p(a("@decarbnow", href = "https://twitter.com/DecarbNow", target = "_blank"), "| data from ", a("@CopernicusECMWF", href = "https://twitter.com/CopernicusECMWf", target = "_blank")),
...
server = function(input, output, session) {
    ...
    plot.label = reactive({
        if (indicatorInput() == "Index (Jan 2016 = 1.0)"){
            "surface concentration, Jan 2016 = 1.0"
    ...
    outputPlot2 = reactive({
        if(is.null(cityInput())){
            ...
            expand_days = 33 - max(select.df()[year == max(year)][monyear == monYearInput()]$day)
            colLines = unique(data.frame(colors = plot.df()[basetime >= "2016-01-01" & variable %in% c("365d mean") &
                                                                name %in% cityInput()]$colors,
                                         ltypes = plot.df()[basetime >= "2016-01-01" & variable %in% c("365d mean") &
                                                                name %in% cityInput()]$ltypes))
            
            gg = ggplot(plot.df()[basetime >= "2016-01-01" & variable %in% c("365d mean") & 
                                      name %in% cityInput()], 
                ...
                #          y = max(plot.df()[basetime >= "2016-01-01" & variable %in% c("365d mean") & 
                ...
                labs(title = "CAMS NO2 daily mean analysis", 
                     ...
                     subtitle = "Comparison of 365 days running mean (log)")
            ...
            if(indicatorInput() == monthlyRaceName){
                ...
                if(maxDay == as.Date(as.yearmon(maxDay, "%b%Y"), frac = 1)){
                    vLineColor = "red"
                    vLineType = "solid"
                    winner = unique(plot.df()[basetime == maxDay & variable %in% c("365d mean")][value == min(value)]$name)
                    print(winner)
                    
                    gg = gg +
                        annotate("label", x = maxDay-15, 
                                 y = plot.df()[basetime == maxDay & variable %in% c("365d mean")][value == min(value)]$value+0.005,
                                 label = winner,
